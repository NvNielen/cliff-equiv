...
#This function takes a tableau and finds the generators which it represents
def tableau_to_gen(tab):
    n = tab.getTableauSize()
    tableau = tab.getTableau()
    generators=[]
    for i in range(n): #We iterate one row at a time. Each row represents a generator
        r=tableau[n+i,2*n] #First generator is at the row n (n+1 in matrix indexing)
        if r==1:
            gen="-"
        elif r==0:
            gen="+"
